...
const ui = {
  ...
  sceneLayoutSelect: document.getElementById("sceneLayoutSelect"),
...
function setupVrButton() {
  const mount = document.getElementById("enterVrMount");
  if (!mount) {
    console.error("Missing #enterVrMount");
    return;
  }
  ...
  vrButton.classList.add("real-vr-button");

  // Do not use display:none.
  // Keep the real Three.js VRButton visible and clicked directly by the user.
  // This preserves browser user activation for Chrome / PCVR / Quest Link.
  ...
  renderer.xr.addEventListener("sessionstart", () => {
    ...
    setRenderState("Immersive VR", "Stereo cubemap room is active.");
...
function setupEvents() {
  ...
  ui.sceneLayoutSelect?.addEventListener("change", () => {
    state.stereoLayout = ui.sceneLayoutSelect.value;
  });

  ui.previewEyeSelect.value = state.previewMode;
...
async function handleSceneUpload() {
  ...
  const stereoLayout = ui.sceneLayoutSelect?.value || "top-bottom";
  state.stereoLayout = stereoLayout;

  try {
    const imageBitmap = await createImageBitmap(file);
    const meta = validateStereoCubemapImage(imageBitmap.width, imageBitmap.height, stereoLayout);
...
function validateStereoCubemapImage(width, height, stereoLayout = "top-bottom") {
  ...
  if (stereoLayout === "side-by-side") {
    if (width % 12 !== 0) {
      throw new Error("Side-by-side stereo cubemap format error: full image width must contain 12 horizontal square faces.");
    }

    const faceSize = width / 12;
    if (faceSize !== height) {
      throw new Error(
        `Side-by-side stereo cubemap format error: expected square faces. Detected width/12 = ${faceSize} and height = ${height}.`,
      );
    }

    return { faceSize, stereoLayout, eyeStripWidth: faceSize * 6, eyeStripHeight: height };
  }

  if (height % 2 !== 0) {
    throw new Error("Top-bottom stereo cubemap format error: image height must split cleanly into top and bottom eye rows.");
  }

  const eyeRowHeight = height / 2;
  if (width % 6 !== 0) {
    throw new Error("Top-bottom stereo cubemap format error: each eye row must contain exactly 6 horizontal square faces.");
  }

  const faceSize = width / 6;
  if (faceSize !== eyeRowHeight) {
    throw new Error(
      `Top-bottom stereo cubemap format error: expected square faces. Detected width/6 = ${faceSize} and height/2 = ${eyeRowHeight}.`,
    );
  }

  return { faceSize, stereoLayout: "top-bottom", eyeStripWidth: width, eyeStripHeight: eyeRowHeight };
}

function getStereoLayoutLabel(stereoLayout) {
  return stereoLayout === "side-by-side" ? "Side-by-side" : "Top-bottom";
}
...
function renderSceneLibrary() {
  ...
  state.scenes
    ...
    .forEach((sceneRecord) => {
      ...
      const layoutLabel = getStereoLayoutLabel(sceneRecord.stereoLayout);
      meta.textContent = `${sceneRecord.width} x ${sceneRecord.height} - ${layoutLabel} - ${sceneRecord.fileName}`;
...
async function loadDemoScene() {
  ...
  state.stereoLayout = demoMeta.stereoLayout;
  if (ui.sceneLayoutSelect) {
    ui.sceneLayoutSelect.value = demoMeta.stereoLayout;
  }
  updateSceneDetails();
  syncFaceConfigUi();
...
async function loadSceneRecord(sceneRecord) {
  try {
    ...
    const stereoLayout = sceneRecord.stereoLayout || "top-bottom";
    const meta = validateStereoCubemapImage(imageBitmap.width, imageBitmap.height, stereoLayout);
    const settings = {
    ...
    state.stereoLayout = stereoLayout;
    if (ui.sceneLayoutSelect) {
      ui.sceneLayoutSelect.value = stereoLayout;
    }
    ui.imageStatusPill.textContent = `${meta.faceSize}px ${getStereoLayoutLabel(stereoLayout)}`;
    clearError();
  } catch (error) {
    console.error(error);
...
function splitStereoCubemapImage(imageSource, sceneMeta) {
  ...
  const stereoLayout = sceneMeta.stereoLayout || "top-bottom";
  const { faceSize } = validateStereoCubemapImage(width, height, stereoLayout);
  ...
  if (stereoLayout === "side-by-side") {
    // Side-by-side split:
    // left half of the uploaded image is the left-eye 6x1 strip,
    // right half is the right-eye 6x1 strip.
    rawLeftFaces = extractFaceRow(imageSource, 0, faceSize, 0);
    rawRightFaces = extractFaceRow(imageSource, 0, faceSize, faceSize * 6);
  } else {
    // Top-bottom split:
    // top half of the uploaded image is the left-eye 6x1 strip,
    // bottom half is the right-eye 6x1 strip.
    rawLeftFaces = extractFaceRow(imageSource, 0, faceSize, 0);
    rawRightFaces = extractFaceRow(imageSource, faceSize, faceSize, 0);
  }
...
function extractFaceRow(imageSource, sourceY, faceSize, sourceXOffset = 0) {
  const faces = [];
...
function updateSceneDetails() {
  ...
  const sceneMeta = state.currentMeta
    ? `${state.currentMeta.width} x ${state.currentMeta.height} - ${getStereoLayoutLabel(state.currentMeta.stereoLayout)} - ${state.currentMeta.fileName}`
